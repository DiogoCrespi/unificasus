-- ============================================================================
-- Script para DELETAR duplicatas de CID10 preservando a MELHOR descrição
-- Estratégia: 
-- 1. Se tem MAIÚSCULAS, mantém a primeira MAIÚSCULA (sistema antigo)
-- 2. Se não tem MAIÚSCULAS, mantém a com maior descrição (mais completa)
-- 3. Se empate, mantém o menor INDICE
-- ============================================================================
...
-- 2. DELETAR duplicatas preservando a melhor descrição
DELETE FROM RL_PROCEDIMENTO_CID
WHERE DT_COMPETENCIA = '202510'
  AND INDICE NOT IN (
      -- Mantém o registro com melhor descrição
      SELECT 
          MIN(
              CASE 
                  -- Se tem MAIÚSCULAS, pega o menor INDICE entre elas
                  WHEN EXISTS (
                  ...
                  THEN CASE 
                      ...
                      ELSE 999999999
                  END
                  -- Se não tem MAIÚSCULAS, pega o com maior descrição (mais completa)
                  ELSE pc.INDICE
              END
          )
